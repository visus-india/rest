			...
			<div class="row">
				<div class="col-md-6">
					...
					<div style="padding-left: 0px;padding-right: 0px;" class="form-group col-xs-3" >
						<label >&nbsp;</label>
						<input type="text" class="form-control"  value = ""  name = "minsalary" id="minsalary" 
					...
					<div style="padding-left: 0px;padding-right: 0px;" class="form-group col-xs-3">
						<label >&nbsp;</label>
						<input type="text" class="form-control"  value = ""  name = "maxsalary" id="maxsalary" 
			...
			<div class="row">
				...
				<div class="col-md-3"> 
					<div class="form-group">
						...
						<input type="text" class="form-control" name ="expectedJoinDate" id="expectedJoinDate" value = ""  placeholder="Expected Join Date" onblur ="validateExpectedJoinDate();" readonly style="background-color:white;  cursor: pointer;">
					</div>
				</div>
				<div class="col-md-3"> 
					<div class="form-group">
...
<script>
	...
	function validateExpectedJoinDate()
	{
		...
		var jobOpenDate = document.getElementById('jobOpenDate').value;
		
		if ( jobOpenDate != "" ||jobOpenDate != '00/00/0000' )
		{
			alert("in if");
			jobOpenDate = new Date (jobOpenDate);
		}
		else
		{
		alert("else");
			jobOpenDate = new Date();
		}
...
<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		//R4S salary accepts non numeric value change. Salary is modified to text after UAT.
		...
		$expectedJoinDate ,$jobCurrency,$salaryType,$jobTypeId,$jobCode,$jobClosedCheck)
		{
			...
			try
			{
				...
				values 
				(now(), now(),
				STR_TO_DATE('$jobOpendate','%d/%m/%Y'),
				STR_TO_DATE('$applyBefore','%d/%m/%Y'),
				STR_TO_DATE('$expectedJoinDate','%d/%m/%Y'),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				$q = $pdo->prepare($sql);	
